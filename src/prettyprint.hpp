...
struct delimiters_values {
  ...
  char_type const* prefix;
  char_type const* delimiter;
  char_type const* postfix;
};
...
struct print_container_helper {
  ...
  template<typename U>
  struct printer {
    static void print_body(U const& c, ostream_type& stream) {
      using std::begin;
      using std::end;

      auto it = begin(c);
      auto const the_end = end(c);
  ...
  print_container_helper(T const& container) : container_(container) {}
...
struct print_container_helper<T, TChar, TCharTraits, TDelimiters>::printer<
  ...
  static void print_body(std::pair<T1, T2> const& c, ostream_type& stream) {
    stream << c.first;
    if (print_container_helper<T, TChar, TCharTraits, TDelimiters>::
          delimiters_type::values.delimiter != NULL)
      stream << print_container_helper<T, TChar, TCharTraits, TDelimiters>::
...
struct print_container_helper<T, TChar, TCharTraits, TDelimiters>::printer<
  ...
  static void print_body(element_type const& c, ostream_type& stream) {
    tuple_print(c, stream, Int<0>());
  }

  static void
  tuple_print(element_type const&, ostream_type&, Int<sizeof...(Args)>) {}

  static void tuple_print(element_type const& c,
                          ostream_type& stream,
  ...
  static void tuple_print(element_type const& c, ostream_type& stream, Int<N>) {
    if (print_container_helper<T, TChar, TCharTraits, TDelimiters>::
          delimiters_type::values.delimiter != NULL)
      stream << print_container_helper<T, TChar, TCharTraits, TDelimiters>::
                  delimiters_type::values.delimiter;
...
inline std::basic_ostream<TChar, TCharTraits>& operator<<(
  ...
  print_container_helper<T, TChar, TCharTraits, TDelimiters> const& helper) {
  helper(stream);
  return stream;
}
...
struct delimiters<T, char> {
  static delimiters_values<char> const values;
};
template<typename T>
delimiters_values<char> const delimiters<T, char>::values = {"[", ", ", "]"};
template<typename T>
struct delimiters<T, wchar_t> {
  static delimiters_values<wchar_t> const values;
};
template<typename T>
delimiters_values<wchar_t> const delimiters<T, wchar_t>::values = {
  L"[", L", ", L"]"};
...
template<typename T, typename TComp, typename TAllocator>
delimiters_values<wchar_t> const
...
template<typename T, typename TComp, typename TAllocator>
delimiters_values<char> const delimiters<::std::multiset<T, TComp, TAllocator>,
                                         char>::values = {"{", ", ", "}"};
...
template<typename T1, typename T2>
delimiters_values<char> const delimiters<std::pair<T1, T2>, char>::values = {
  "(", ", ", ")"};
template<typename T1, typename T2>
...
template<typename... Args>
delimiters_values<char> const delimiters<std::tuple<Args...>, char>::values = {
  "(", ", ", ")"};
template<typename... Args>
...
struct custom_delims_wrapper : custom_delims_base {
  custom_delims_wrapper(T const& t_) : t(t_) {}
  ...
  std::wostream& stream(std::wostream& s) {
    return s << print_container_helper<T,
                                       wchar_t,
...
struct custom_delims {
  ...
  custom_delims(Container const& c)
...
struct array_wrapper_n {
  ...
  array_wrapper_n(T const* const a, size_t n) : _array(a), _n(n) {}
  inline const_iterator begin() const { return _array; }
  inline const_iterator end() const { return _array + _n; }

private:
  T const* const _array;
  size_t _n;
};
...
struct bucket_print_wrapper {
  ...
  bucket_print_wrapper(T const& m, size_type bucket) : m_map(m), n(bucket) {}
...
template<typename T>
inline pretty_print::array_wrapper_n<T> pretty_print_array(T const* const a,
                                                           size_t n) {
  return pretty_print::array_wrapper_n<T>(a, n);
}

template<typename T>
pretty_print::bucket_print_wrapper<T> bucket_print(T const& m,
